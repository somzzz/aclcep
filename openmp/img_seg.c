...
int main(int argc, char *argv[]) {
	...
	ScanPgm(&image);
	ScanPgm(&binary);
	name_file = strrchr(image.name, '/');
	strncpy(name_file, name_file + 1, strlen(name_file) - 1);
	name_pic = strtok(name_file, ".");
	strcat(name_pic, ".txt");
	if (stat(name_pic, &sb) == -1)
	{
		output_data = fopen(name_pic, "a+");
		fprintf(output_data, "\tName_pic\tSize_pic\tThreads\tChunck\tSquare\tTime\n");
	} else {
		output_data = fopen(name_pic, "a+");
	}	
	...
	fprintf(output_data, "\t%s\t%dX%d\t%d\t%d\t%d\t%lf\n"
	, name_pic, binary.xdim, binary.ydim, nr_threads, chuncksize, latura, elapsed_time);	
	fclose(output_data);
	return 0;
}
